...
const detectors: Record<string, Det> = {
  ...
  // 1.3 Billed above actual demand — genuine over-billing ONLY when billing
  // demand exceeds both recorded MD and the tariff's contract-demand floor
  // (~75% of CD). Without CD we can't tell an error from a legitimate floor, so
  // we don't flag it (avoids false-positives on normal floor-based billing).
  "1.3": (n) => {
    const bd = n("billingDemandKva");
    const md = n("maxDemandKva");
    const cd = n("contractDemandKva");
  ...
  // 2.3 Lost high-PF rebate — only when PF already ≥ 0.95 and no penalty line.
  "2.3": (n) => {
    const pf = pf01(n);
    const fc = n("fixedDemandCharges");
  ...
  // 3.4 ToD not applied — only for ToD-ELIGIBLE consumers (HT supply or ≥100 kW).
  // Many LT/small consumers have no ToD option, so a missing ToD split isn't a
  // loss for them — gating on eligibility avoids a false positive.
  "3.4": (n, fields) => {
  ...
  // 6.5 Arrears — money OWED, not a saving. Flagged for review but contributes
  // ₹0 to recoverable savings (no annualINR), so it never inflates the headline.
  "6.5": (n) => {
    const a = n("arrears");
    return a > 0
